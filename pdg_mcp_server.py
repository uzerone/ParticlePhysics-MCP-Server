            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "properties": {
                        ...
                        "items": {
                            "type": "string",
                            "enum": ["mass", "lifetime", "charge", "spin", "quantum_numbers"]
                        },
    ...
    try:
        ...
        if include_basic:
            ...
            try:
                ...
                info["mass_error"] = f"±{particle.mass_error:.6f} GeV" if particle.mass_error else "N/A"
            except:
                info["mass"] = "N/A"
                info["mass_error"] = "N/A"
            
            try:
                info["lifetime"] = f"{particle.lifetime:.2e} s" if particle.lifetime else "N/A"
            except:
                info["lifetime"] = "N/A"
                ...
                for mass_prop in particle.masses():
                    for measurement in mass_prop.get_measurements():
                        if hasattr(measurement, 'reference') and measurement.reference:
                            measurements.append({
                                "property": "mass",
                                "value": measurement.get_value().value_text if measurement.get_value() else "N/A",
                                "reference": {
                                    "title": getattr(measurement.reference, 'title', 'N/A'),
                                    "doi": getattr(measurement.reference, 'doi', 'N/A'),
        ...
        return {"name": str(particle), "error": f"Error formatting particle info: {str(e)}"}
    ...
    try:
        ...
        if name == "search_particle":
            ...
            try:
                ...
                info = format_particle_info(particle, include_basic=True, include_measurements=include_measurements)
                ...
                return [types.TextContent(
                    type="text",
                    text=json.dumps({"error": f"Failed to get particle properties: {str(e)}"}, indent=2)
                )]
            ...
            try:
                ...
                return [types.TextContent(
                    type="text",
                    text=json.dumps({"error": f"Failed to get branching fractions: {str(e)}"}, indent=2)
                )]
            ...
            try:
                ...
                if hasattr(all_particles, '__iter__') and not isinstance(all_particles, str):
                ...
                for particle in particle_list:
                    ...
                    if particle_type != "all":
                        ...
                        if hasattr(particle, type_method) and not getattr(particle, type_method):
                            continue
                ...
                return [types.TextContent(
                    type="text",
                    text=json.dumps({"error": f"Failed to list particles: {str(e)}"}, indent=2)
                )]
            ...
            try:
                ...
                return [types.TextContent(
                    type="text",
                    text=json.dumps({"error": f"Failed to get particle by MCID: {str(e)}"}, indent=2)
                )]
            ...
            try:
                ...
                for name in particle_names:
                    try:
                        ...
                        for prop in properties:
                            if prop == "mass":
                                try:
                                    particle_info["mass"] = f"{particle.mass:.6f} GeV" if particle.mass else "N/A"
                                except:
                                    particle_info["mass"] = "N/A"
                            elif prop == "lifetime":
                                try:
                                    particle_info["lifetime"] = f"{particle.lifetime:.2e} s" if particle.lifetime else "N/A"
                                except:
                                ...
                                try:
                                    particle_info["spin"] = str(getattr(particle, 'quantum_J', 'N/A'))
                                except:
                                    particle_info["spin"] = "N/A"
                            elif prop == "quantum_numbers":
                                qn = {}
                                for q in ['quantum_J', 'quantum_P', 'quantum_C', 'quantum_G', 'quantum_I']:
                                    try:
                ...
                return [types.TextContent(
                    type="text",
                    text=json.dumps({"error": f"Failed to compare particles: {str(e)}"}, indent=2)
                )]
            ...
            try:
                ...
                return [types.TextContent(
                    type="text",
                    text=json.dumps({"error": f"Failed to get database info: {str(e)}"}, indent=2)
                )]
