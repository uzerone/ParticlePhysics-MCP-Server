...
class SecurityLogger:
    ...
    def log_validation_failure(
        self, input_type: str, value: str, errors: List[str], source_ip: str = "unknown"
    ):
    ...
    def log_suspicious_activity(
        self, activity_type: str, details: str, source_ip: str = "unknown"
    ):
...
class RateLimiter:
    ...
    async def get_remaining_capacity(self, identifier: str = "global") -> int:
        ...
        async with self.lock:
            ...
            return max(
                0,
                int(config.rate_limit.requests_per_second)
...
class InputValidator:
    ...
    def __init__(self):
        ...
        self.allowed_chars_pattern = re.compile(
            config.security.allowed_characters_pattern
        )
    ...
    def validate_particle_name(
        self, name: str, source_ip: str = "unknown"
    ) -> ValidationResult:
        ...
        if len(name) > config.security.max_query_length:
            errors.append(
                f"Particle name too long (max {config.security.max_query_length} characters)"
            )
    ...
    def validate_pdg_id(
        self, pdg_id: str, source_ip: str = "unknown"
    ) -> ValidationResult:
    ...
    def validate_mcid(
        self, mcid: Union[str, int], source_ip: str = "unknown"
    ) -> ValidationResult:
    ...
    def validate_query_string(
        self, query: str, source_ip: str = "unknown"
    ) -> ValidationResult:
        ...
        if len(query) > config.security.max_query_length:
            errors.append(
                f"Query too long (max {config.security.max_query_length} characters)"
            )
    ...
    async def check_rate_limit(
        self, identifier: str = "global", endpoint: str = "unknown"
    ) -> bool:
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **func_kwargs):
            ...
            if input_type == "particle_name" and "particle_name" in func_kwargs:
                ...
                if not result.is_valid:
                    raise ValueError(
                        f"Invalid particle name: {', '.join(result.errors)}"
                    )
                ...
                result = validator.validate_query_string(
                    func_kwargs["query"], source_ip
                )
                if not result.is_valid:
