...
async def test_mcp_server():
    ...
    result = await pp_mcp_server.handle_call_tool('search_particle', {'query': 'electron'})
    ...
    result = await pp_mcp_server.handle_call_tool('get_particle_properties', {'particle_name': 'electron'})
    data = json.loads(result[0].text)
