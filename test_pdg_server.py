...
async def test_error_handling():
    ...
    for test_case in error_test_cases:
        ...
        try:
            import pdg_mcp_server as mcp_module
            result = await mcp_module.handle_call_tool(test_case["tool"], test_case["args"])
            ...
            if result and len(result) > 0:
                ...
                if (isinstance(response_data, list) and len(response_data) > 0 and "error" in response_data[0]) or \
                   ("error" in response_data or 
                    "No particles found" in str(response_data) or
