...
def setup_pdg_connection():
    ...
    try:
        ...
        print("Error: PDG package not installed. Please install it using: pip install pdg")
        sys.exit(1)
    ...
    if command == "search":
        ...
        result = await api.handle_api_tools(
            "compare_particles",
            {
                ...
                "properties": getattr(args, "properties", ["mass", "lifetime", "charge"])
            },
            api_instance
        )
    ...
    if command == "mass-measurements":
        ...
        result = await data.handle_data_tools(
            "get_measurement_value_details",
            {
                ...
                "include_error_breakdown": getattr(args, "include_error_breakdown", True)
            },
            api_instance
        )
    ...
    if command == "quantum-numbers":
        ...
        result = await particle.handle_particle_tools(
            "analyze_particle_item",
            {
                ...
                "include_associated_particles": getattr(args, "include_associated", True)
            },
            api_instance
        )
    ...
    if command == "convert-advanced":
        ...
        result = await units.handle_units_tools(
            "validate_unit_compatibility",
            {
                "unit1": args.unit1,
                "unit2": args.unit2,
                "explain_incompatibility": getattr(args, "explain_incompatibility", True)
            },
            api_instance
        )
...
Examples:
  ...
  python pdg_cli.py pdg-rounding --value 1.23456 --error 0.00123
    ...
    parser.add_argument("--search-type", choices=["name", "mcid", "pdgid", "auto"], 
                       default="auto", help="Type of search (default: auto)")
    parser.add_argument("--measurements", action="store_true", 
                       help="Include measurements")
    parser.add_argument("--type", choices=["all", "baryon", "meson", "lepton", "boson", "quark"],
    ...
    parser.add_argument("--summary", action="store_true", default=True,
                       help="Include summary values")
    parser.add_argument("--units", help="Units for measurements")
    parser.add_argument("--property-type", choices=["mass", "lifetime", "width", "all"],
                       help="Type of property")
    parser.add_argument("--summary-only", action="store_true",
                       help="Only summary table values")
    ...
    parser.add_argument("--as-text", action="store_true", default=True,
                       help="Return as text format")
    
    # Decay module arguments
    parser.add_argument("--decay-type", choices=["exclusive", "inclusive", "all"],
                       help="Type of decay")
    parser.add_argument("--mode-number", type=int, help="Specific decay mode number")
    parser.add_argument("--subdecays", action="store_true", default=True,
                       help="Include subdecays")
    ...
    parser.add_argument("--check-data", action="store_true", default=True,
                       help="Check data availability")
    parser.add_argument("--suggest", action="store_true", default=True,
                       help="Suggest alternatives")
    parser.add_argument("--error-type", 
                       choices=["all", "PdgApiError", "PdgInvalidPdgIdError", 
                               "PdgNoDataError", "PdgAmbiguousValueError", "PdgRoundingError"],
                       help="Specific error type")
    parser.add_argument("--lookup-type", 
                       choices=["particle_name", "pdg_id", "mcid", "property"],
                       help="Type of lookup")
    parser.add_argument("--suggestions", action="store_true", default=True,
                       help="Include suggestions")
    parser.add_argument("--alternatives", action="store_true", default=True,
                       help="Return alternatives")
    parser.add_argument("--error-details", action="store_true", default=True,
    ...
    parser.add_argument("--include-values", action="store_true", default=True,
                       help="Include measurement values")
    parser.add_argument("--include-reference", action="store_true", default=True,
                       help="Include reference information")
    parser.add_argument("--include-footnotes", action="store_true", default=True,
                       help="Include footnotes")
    parser.add_argument("--include-error-breakdown", action="store_true", default=True,
                       help="Include detailed error breakdown")
    parser.add_argument("--include-doi", action="store_true", default=True,
    ...
    parser.add_argument("--include-associated", action="store_true", default=True,
                       help="Include associated particles")
    parser.add_argument("--quantum-numbers", nargs="+",
                       choices=["J", "P", "C", "G", "I", "all"],
                       help="Quantum numbers to compare")
    parser.add_argument("--include-asymmetric", action="store_true", default=True,
                       help="Include asymmetric error information")
    
    # Units module arguments
    parser.add_argument("--validate-compatibility", action="store_true", default=True,
                       help="Validate unit compatibility")
    parser.add_argument("--unit-type", choices=["all", "energy", "time"],
                       help="Filter by unit type")
    parser.add_argument("--include-factors", action="store_true", default=True,
                       help="Include conversion factors")
    parser.add_argument("--constant-name", choices=["all", "hbar", "hbar_gev_s"],
                       help="Specific physics constant")
    parser.add_argument("--include-description", action="store_true", default=True,
    ...
    parser.add_argument("--explain-incompatibility", action="store_true", default=True,
    ...
    parser.add_argument("--conversion-type", 
                       choices=["energy_to_length", "energy_to_time", "mass_to_energy",
                               "length_to_energy", "time_to_energy", "energy_to_mass"],
    ...
    parser.add_argument("--category", choices=["all", "energy", "time", "mass", "length"],
    ...
    parser.add_argument("--link-type", choices=["measurements", "references", "footnotes", "values"],
                       help="Type of linked data")
    parser.add_argument("--data-input", help="Data to normalize")
    parser.add_argument("--data-type", choices=["particle_name", "pdg_id", "value", "measurement"],
                       help="Type of data to normalize")
    parser.add_argument("--strict", action="store_true", default=False,
                       help="Use strict normalization")
...
async def async_main():
    ...
    api_commands = ["search", "properties", "list", "compare", "database-info", 
                   "canonical-name", "editions"]
    data_commands = ["mass-measurements", "lifetime-measurements", "width-measurements",
                    "summary-values", "convert-units", "data-type-keys", "value-type-keys"]
    decay_commands = ["branching-fractions", "decay-products", "branching-ratios",
                     "decay-analysis", "decay-details"]
    error_commands = ["validate", "error-info", "diagnose", "safe-lookup"]
    measurement_commands = ["measurement-details", "value-details", "reference-details", 
                           "search-by-reference", "footnote-details", "analyze-errors",
                           "measurements-for-particle", "compare-techniques"]
    particle_commands = ["quantum-numbers", "check-properties", "particle-list-criteria",
                        "properties-detailed", "analyze-item", "mass-details", 
                        "lifetime-details", "width-details", "compare-quantum-numbers",
                        "particle-error-info"]
    units_commands = ["convert-advanced", "unit-factors", "physics-constants",
                     "validate-compatibility", "unit-info", "natural-units",
                     "common-conversions"]
    utils_commands = ["parse-pdg-id", "base-pdg-id", "make-pdg-id", "find-best-property",
                     "pdg-rounding", "linked-data", "normalize-data", "table-data"]
