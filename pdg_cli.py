...
async def search_particle(query: str, search_type: str = "auto") -> None:
    ...
    result = await server.handle_call_tool(
        "search_particle", {"query": query, "search_type": search_type}
    )
...
async def get_particle_properties(
    particle_name: str, include_measurements: bool = False
) -> None:
    """Get particle properties"""
    result = await server.handle_call_tool(
        "get_particle_properties",
        {"particle_name": particle_name, "include_measurements": include_measurements},
    )
    data = json.loads(result[0].text)
...
async def get_branching_fractions(
    particle_name: str, decay_type: str = "exclusive", limit: int = 20
) -> None:
    """Get branching fractions"""
    result = await server.handle_call_tool(
        "get_branching_fractions",
        {"particle_name": particle_name, "decay_type": decay_type, "limit": limit},
    )
    data = json.loads(result[0].text)
...
async def list_particles(particle_type: str = "all", limit: int = 50) -> None:
    """List particles"""
    result = await server.handle_call_tool(
        "list_particles", {"particle_type": particle_type, "limit": limit}
    )
    data = json.loads(result[0].text)
    ...
    if "error" in data:
        ...
        for particle in data["particles"]:
            print(
                f"  {particle['name']:<15} MCID: {str(particle.get('mcid', 'N/A')):<10} Mass: {particle.get('mass', 'N/A')}"
            )


async def get_particle_by_mcid(mcid: int) -> None:
    """Get particle by Monte Carlo ID"""
    result = await server.handle_call_tool("get_particle_by_mcid", {"mcid": mcid})
    data = json.loads(result[0].text)
...
async def compare_particles(
    particle_names: List[str], properties: List[str] = None
) -> None:
    """Compare particles"""
    ...
    result = await server.handle_call_tool(
        "compare_particles",
        {"particle_names": particle_names, "properties": properties},
    )
    data = json.loads(result[0].text)
...
def main():
    ...
    search_parser.add_argument(
        "--query", required=True, help="Particle name, MCID, or PDG ID"
    )
    search_parser.add_argument(
        "--type",
    ...
    br_parser.add_argument(
        "--type",
        choices=["exclusive", "inclusive", "all"],
        default="exclusive",
        help="Decay type",
    )
    ...
    compare_parser.add_argument(
        "--particles", nargs="+", required=True, help="Particle names"
    )
    compare_parser.add_argument(
        "--properties",
        nargs="+",
