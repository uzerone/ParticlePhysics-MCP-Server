...
def test_module_tools():
    ...
    try:
        ...
        total_tools = len(api_tools) + len(data_tools) + len(decay_tools) + len(error_tools) + len(measurement_tools) + len(particle_tools) + len(units_tools) + len(utils_tools)
...
def test_json_config():
    ...
    try:
        ...
        if os.path.exists(config_file):
            ...
            assert 'particlephysics' in data['mcpServers'], 'Missing particlephysics server'
