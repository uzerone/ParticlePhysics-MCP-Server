...
async def example_lepton_family_study():
    ...
    # Compare leptons
    comparison_result = await call_tool("compare_particles", {
    ...
    print("\nTau decay modes:")
    tau_decays = await call_tool("get_branching_fractions", {
        "particle_name": "tau-",
...
async def example_meson_decay_analysis():
    ...
    for kaon in kaon_types:
        ...
        decays = await call_tool("get_branching_fractions", {
            "particle_name": kaon,
            "decay_type": "exclusive", 
            "limit": 5
        })
...
async def example_heavy_quark_physics():
    ...
    comparison = await call_tool("compare_particles", {
        "particle_names": b_mesons,
    ...
    if "error" not in comparison:
        for particle in comparison["particles"]:
            if "error" not in particle:
                print(f"  {particle['name']:<6}: Mass = {particle.get('mass', 'N/A'):<15} Lifetime = {particle.get('lifetime', 'N/A')}")
    
    # Look at B+ decay modes involving J/psi
    print(f"\nB+ decays involving J/psi:")
    b_decays = await call_tool("get_branching_fractions", {
        "particle_name": "B+",
        "decay_type": "exclusive",
        "limit": 50
    })
    ...
    if "error" not in b_decays:
        jpsi_decays = [decay for decay in b_decays["decay_modes"] 
...
async def example_particle_discovery_timeline():
    ...
    for category, particles in fundamental_particles.items():
        ...
        for particle_name in particles:
            properties = await call_tool("get_particle_properties", {
...
async def example_database_exploration():
    ...
    for ptype in particle_types:
        particles = await call_tool("list_particles", {
            "particle_type": ptype,
            "limit": 100
        })
