...
def format_branching_fraction_value(
    bf_value: Any, include_uncertainty: bool = True
) -> Dict[str, Any]:
    ...
    try:
        ...
        if include_uncertainty and result["value"] is not None:
            ...
            if error_pos is not None or error_neg is not None or error_sym is not None:
                ...
                if error_pos == error_neg or (
                    error_pos is not None and error_neg is None
                ):
                    ...
                    if total_error and result["value"] != 0:
                        uncertainty_info["relative_error"] = abs(
                    ...
                    if error_pos and error_neg and result["value"] != 0:
                        ...
                        uncertainty_info["relative_error"] = avg_error / abs(
                            result["value"]
                        )
...
def classify_decay_mode(
    decay_description: str, decay_products: List[Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            product_count = (
                len(decay_products) if decay_products else desc_lower.count(" ") + 1
            )
...
def analyze_decay_conservation(
    parent_particle: Any, decay_products: List[Any]
) -> Dict[str, Any]:
        ...
        try:
            ...
            if product_charges:
                ...
                conservation_analysis["charge_conservation"] = {
                    "status": (
                        "conserved"
                        if abs(parent_charge - total_product_charge) < 0.1
                        else "violated"
                    ),
                    "details": {
...
def format_enhanced_decay_product(
    decay_product: Any, include_particle_details: bool = True, subdecay_depth: int = 0
) -> Dict[str, Any]:
                ...
                if particles:
                    ...
                    for particle in particles[:5]:  # Limit to first 5 particles
                        ...
                        particle_info["classification"] = {
                            "is_lepton": safe_get_attribute(
                                particle, "is_lepton", False
                            ),
                            "is_baryon": safe_get_attribute(
                    ...
                    total_charge = sum(
                        ...
                        if p.get("charge") is not None
                    )
                    product_info["total_charge"] = (
                        total_charge * product_info["multiplier"]
                    )
        ...
        if subdecay:
            try:
                subdecay_info = {
                    ...
                    "subdecay_level": safe_get_attribute(
                        subdecay, "subdecay_level", subdecay_depth + 1
                    ),
                ...
                if subdecay_info["branching_fraction"] is not None:
                    subdecay_info["formatted_bf"] = format_branching_fraction_value(
                        subdecay
                    )
                ...
                product_info["subdecay_analysis"] = {
                    "error": f"Subdecay analysis failed: {str(e)}"
                }
...
def format_enhanced_branching_fraction(
    bf: Any, include_analysis: bool = True, include_products: bool = True
) -> Dict[str, Any]:
    ...
    try:
        ...
        bf_value_analysis = format_branching_fraction_value(
            bf, include_uncertainty=include_analysis
        )
        ...
        if include_analysis:
            try:
                products = (
                    list(bf.decay_products) if hasattr(bf, "decay_products") else []
                )
        ...
        if include_products:
            try:
                ...
                if products:
                    ...
                    total_charge = sum(
                        p.get("total_charge", 0)
                        for p in products
                        if p.get("total_charge") is not None
                    )
        ...
        bf_info["data_quality"] = {
            "has_uncertainty": bf_value_analysis.get("uncertainty_analysis", {}).get(
                "has_uncertainty", False
            ),
...
def calculate_decay_statistics(
    branching_fractions: List[Dict[str, Any]],
) -> Dict[str, Any]:
    ...
    try:
        ...
        for bf in branching_fractions:
            ...
            stats["decay_type_distribution"][decay_type] = (
                stats["decay_type_distribution"].get(decay_type, 0) + 1
            )

            # Multiplicity distribution
            multiplicity = bf.get("total_final_state_multiplicity", 0)
            if multiplicity > 0:
                stats["multiplicity_distribution"][str(multiplicity)] = (
                    stats["multiplicity_distribution"].get(str(multiplicity), 0) + 1
                )
...
def build_decay_tree(
    particle_name: str,
    branching_fractions: List[Any],
    max_depth: int = 3,
    current_depth: int = 0,
) -> Dict[str, Any]:
            ...
            try:
                for product in bf.decay_products:
                    product_node = {
                        ...
                        "multiplier": safe_get_attribute(product, "multiplier", 1),
                        "has_subdecay": safe_get_attribute(product, "subdecay")
                        is not None,
                    }
    ...
    try:
        if name == "get_branching_fractions":
            ...
            try:
                ...
                if decay_type in ["exclusive", "all"]:
                    try:
                        ...
                        logger.debug(
                            f"Error getting exclusive branching fractions: {e}"
                        )
                ...
                if decay_type in ["inclusive", "all"]:
                    try:
                        ...
                        logger.debug(
                            f"Error getting inclusive branching fractions: {e}"
                        )
                ...
                for bf_type, bf in all_bfs:
                    try:
                        ...
                        if bf_value is not None and bf_value >= min_bf:
                            ...
                            "is_limit", False
                        ):  # Include limits regardless of threshold
                            formatted_bfs.append(bf_info)
                ...
                if sort_by == "branching_fraction":
                    ...
                    formatted_bfs.sort(
                        key=lambda x: x.get("total_final_state_multiplicity", 0)
                    )
            ...
            try:
                ...
                if decay_type in ["exclusive", "all"]:
                    try:
                        for bf in list(particle.exclusive_branching_fractions())[
                            :max_modes_per_level
                        ]:
                            bf_value = safe_get_attribute(bf, "value", 0)
                            if bf_value >= min_probability or safe_get_attribute(
                                bf, "is_limit", False
                            ):
                ...
                if include_visualization:
                    decay_tree = build_decay_tree(
                        particle_name, branching_fractions, max_depth
                    )
                ...
                for i, bf in enumerate(branching_fractions):
                    try:
                        bf_analysis = format_enhanced_branching_fraction(
                            bf, include_analysis=True, include_products=True
                        )
                        ...
                        if include_probability_flow:
                            ...
                            if bf_value > 0:
                                ...
                                for product in bf_analysis.get("decay_products", []):
                                    ...
                                    if subdecay_info and subdecay_info.get(
                                        "branching_fraction"
                                    ):
                                        contrib_prob = (
                                            cumulative_prob
                                            * subdecay_info["branching_fraction"]
                                        )
                                        subdecay_contributions.append(
                                            {
                                                "product": product["item_name"],
                                                "subdecay_probability": subdecay_info[
                                                    "branching_fraction"
                                                ],
                        ...
                        logger.debug(
                            f"Error analyzing decay structure for mode {i}: {e}"
                        )
                ...
                if include_pattern_analysis:
                    ...
                    for bf_analysis in structure_analysis:
                        ...
                        patterns["decay_signatures"][decay_type_class] = (
                            patterns["decay_signatures"].get(decay_type_class, 0) + 1
                        )

                        # Count final state multiplicities
                        multiplicity = bf_analysis.get(
                            "total_final_state_multiplicity", 0
                        )
                        if multiplicity > 0:
                            patterns["common_final_states"][f"{multiplicity}_body"] = (
                                patterns["common_final_states"].get(
                                    f"{multiplicity}_body", 0
                                )
                                + 1
                            )
