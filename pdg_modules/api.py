...
This module provides the foundational interface to the Particle Data Group (PDG) database,
offering comprehensive tools for particle discovery, identification, and data retrieval.
It serves as the primary gateway for accessing particle physics data through the PDG Python API.

Key Features:
- Advanced particle search with intelligent auto-detection and fuzzy matching
- Comprehensive particle property retrieval with enhanced metadata
- Multi-particle comparison with statistical analysis and uncertainty propagation
- Database information and edition management
- Canonical name resolution and identifier validation
- Particle listing with advanced filtering and pagination
- Monte Carlo ID lookup with related particle discovery
- Edition-aware data retrieval with version control

Core Tools (11 total):
1. search_particle - Advanced particle search by name, MCID, or PDG ID
2. get_particle_properties - Comprehensive particle properties with metadata
3. list_particles - Advanced particle listing with filtering and sorting
4. get_particle_by_mcid - Monte Carlo ID lookup with validation
5. compare_particles - Multi-particle comparison with statistical analysis
6. get_database_info - PDG database metadata and statistics
7. get_canonical_name - Canonical name resolution with alternatives
8. get_particles_by_name - Name-based search with fuzzy matching
9. get_editions - PDG Review editions with comprehensive metadata
10. get_pdg_by_identifier - Enhanced PDG object retrieval with validation
11. get_all_pdg_identifiers - Filtered identifier listing with pagination

Enhanced Capabilities:
- Intelligent search type detection (auto, name, mcid, pdgid, fuzzy)
- Comprehensive error handling with detailed diagnostics
- Advanced particle classification and type detection
- Quantum number formatting with proper notation
- Statistical analysis for multi-particle comparisons
- Enhanced metadata extraction with quality indicators
- Antiparticle support and related particle discovery
- Educational content with physics context and explanations

Data Quality Features:
- Uncertainty analysis and error propagation
- Measurement validation and quality assessment
- Reference tracking and citation management
- Data freshness indicators and edition awareness
- Comprehensive logging for debugging and analysis

Integration Points:
- Seamless integration with other PDG modules (data, decay, measurement, etc.)
- MCP (Model Context Protocol) compatible tool definitions
- JSON-formatted responses for easy parsing and integration
- Extensible architecture for future enhancements

Based on the official PDG Python API: https://github.com/particledatagroup/api
Enhanced for comprehensive particle physics research and AI assistant integration.
        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "particle_type": {
                        "type": "string",
                        "enum": ["all", "baryon", "meson", "lepton", "boson", "quark", "composite", "fundamental"],
                        "default": "all",
                    ...
                    "mass_range": {
                        "type": "object",
                        "properties": {
                            "min_mass": {"type": "number", "description": "Minimum mass in GeV"},
                            "max_mass": {"type": "number", "description": "Maximum mass in GeV"},
                        },
...
def format_value_with_uncertainty(value: Any, error_pos: Any = None, error_neg: Any = None, units: str = "") -> Dict[str, Any]:
        ...
        if result["value"] is not None:
            if error_pos is not None or error_neg is not None:
                if error_pos == error_neg or error_neg is None:
                    result["formatted"] = f"{result['value']:.6g} ± {error_pos:.6g} {units}".strip()
                    result["uncertainty"] = {"symmetric": float(error_pos)}
                else:
                    result["formatted"] = f"{result['value']:.6g} +{error_pos:.6g}/-{abs(error_neg):.6g} {units}".strip()
...
def get_particle_classification(particle: Any) -> Dict[str, Any]:
    ...
    try:
        ...
        if safe_get_attribute(particle, "is_lepton", False):
            classification.update({"type": "lepton", "category": "fundamental", "is_fundamental": True})
        elif safe_get_attribute(particle, "is_quark", False):
            classification.update({"type": "quark", "category": "fundamental", "is_fundamental": True})
        elif safe_get_attribute(particle, "is_boson", False):
            classification.update({"type": "boson", "category": "fundamental", "is_fundamental": True})
        elif safe_get_attribute(particle, "is_baryon", False):
            classification.update({"type": "baryon", "category": "hadron", "is_composite": True, "constituents": "quarks"})
        elif safe_get_attribute(particle, "is_meson", False):
            classification.update({"type": "meson", "category": "hadron", "is_composite": True, "constituents": "quark-antiquark"})
...
def format_enhanced_particle_info(
    ...
    try:
        ...
        if include_basic:
            ...
            info["mass"] = format_value_with_uncertainty(mass_value, mass_error, units=units_preference)
            
            # Lifetime information
            lifetime_value = safe_get_attribute(particle, "lifetime")
            if lifetime_value:
                info["lifetime"] = format_value_with_uncertainty(lifetime_value, units="s")
                # Calculate mean life if available
                if lifetime_value > 0:
                    info["mean_life"] = format_value_with_uncertainty(lifetime_value, units="s")
            
            # Width information  
            width_value = safe_get_attribute(particle, "width")
            if width_value:
                info["width"] = format_value_with_uncertainty(width_value, units=units_preference)
        ...
        if include_measurements:
            ...
            try:
                ...
                for mass_prop in getattr(particle, "masses", lambda: [])():
                    for measurement in getattr(mass_prop, "get_measurements", lambda: [])():
                        meas_info = {
                            "property": "mass",
                            "value": safe_get_attribute(measurement, "value"),
                            "uncertainty": {
                                "positive": safe_get_attribute(measurement, "error_positive"),
                                "negative": safe_get_attribute(measurement, "error_negative"),
                            },
                            "units": safe_get_attribute(measurement, "units", units_preference),
                        }
        ...
        if include_decays:
            ...
            try:
                ...
                exclusive_bfs = list(getattr(particle, "exclusive_branching_fractions", lambda: [])())
                if exclusive_bfs:
                    ...
                    for bf in exclusive_bfs[:3]:
                        decay_mode = {
                            ...
                            "formatted_bf": safe_get_attribute(bf, "display_value_text"),
                        }
...
def calculate_property_ratios(particles_data: List[Dict], property_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        for p_data in particles_data:
            if property_name in p_data and p_data[property_name].get("value") is not None:
    ...
    try:
        if name == "search_particle":
            ...
            if search_type == "auto":
                if query.isdigit():
                    search_type = "mcid"
                elif any(query.upper().startswith(prefix) for prefix in ["S", "M", "G", "T"]) and any(c.isdigit() for c in query):
                    search_type = "pdgid"
                else:
                    search_type = "name"
            ...
            try:
                if search_type == "name" or search_type == "fuzzy":
                    try:
                        ...
                        try:
                            particles_list = list(api.get_particles_by_name(query, case_sensitive=False))
                            for particle_group in particles_list[:max_results]:
                                if hasattr(particle_group, "__iter__") and not isinstance(particle_group, str):
                                    for particle in particle_group:
                                        if hasattr(particle, "name"):
                                            results.append(format_enhanced_particle_info(particle))
                                elif hasattr(particle_group, "name"):
                                    results.append(format_enhanced_particle_info(particle_group))
                        except Exception as e:
                ...
                results.append({
                    ...
                    "suggestions": ["Check particle name spelling", "Try different search type", "Use fuzzy search"]
                })
                ...
                for name in particle_names:
                    try:
                        ...
                        comparison["particles"].append({
                            "name": name,
                            "error": f"Failed to get particle: {str(e)}"
                        })
            ...
            try:
                ...
                if include_version_info:
                    info.update({
                        ...
                        "default_edition": safe_get_attribute(api, "default_edition", "Unknown"),
                        "available_editions": safe_get_attribute(api, "editions", []),
                    })
                ...
                if include_statistics:
                    try:
                        ...
                        info["statistics"] = {"error": f"Could not retrieve statistics: {e}"}
                
                # Add API capabilities
                info["api_capabilities"] = {
                    "search_methods": ["name", "mcid", "pdgid", "fuzzy"],
                    "data_types": ["mass", "lifetime", "width", "quantum_numbers", "decays"],
        ...
        # [Additional tools would be implemented here with similar enhanced patterns]
            ...
            error_response = {
                ...
                "available_tools": [
                    "search_particle", "get_particle_properties", "compare_particles",
                    "get_database_info", "list_particles", "get_particle_by_mcid",
                    # ... other tools
                ]
            }
            return [types.TextContent(type="text", text=json.dumps(error_response, indent=2))]
