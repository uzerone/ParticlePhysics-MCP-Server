        ...
        types.Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "conversion_type": {
                        "type": "string",
                        "enum": ["energy_to_length", "energy_to_time", "mass_to_energy", 
...
def validate_unit_compatibility(unit1, unit2):
    ...
    if type1 != type2:
        return False, f"Cannot convert between {type1} ({unit1}) and {type2} ({unit2}) units"
...
def convert_units_pdg(value, old_units=None, new_units=None):
    ...
    if old_factor[1] != new_factor[1]:
        raise ValueError(f'Illegal unit conversion from {old_factor[1]} to {new_factor[1]}')
...
async def handle_units_tools(name: str, arguments: dict, api) -> List[types.TextContent]:
    ...
    if name == "convert_units_advanced":
        ...
        try:
            ...
            if validate_compatibility:
                is_compatible, message = validate_unit_compatibility(from_units, to_units)
        ...
        try:
            ...
            return [types.TextContent(
                type="text",
                text=json.dumps({"error": f"Failed to get conversion factors: {str(e)}"}, indent=2)
            )]
        ...
        try:
            if constant_name == "all":
                ...
                if constant_name in constant_map:
                    ...
                    if key in PHYSICS_CONSTANTS:
                        ...
                        return [types.TextContent(
                            type="text",
                            text=json.dumps({"error": f"Constant '{constant_name}' not found"}, indent=2)
                        )]
                else:
                    return [types.TextContent(
                        type="text",
                        text=json.dumps({"error": f"Unknown constant '{constant_name}'"}, indent=2)
                    )]
            
            # Format output
            if not include_description:
                constants = {k: {"value": v["value"], "units": v["units"]} 
            ...
            return [types.TextContent(
                type="text",
                text=json.dumps({"error": f"Failed to get physics constants: {str(e)}"}, indent=2)
            )]
        ...
        try:
            ...
            if is_compatible:
                ...
                try:
                    factor = UNIT_CONVERSION_FACTORS[unit1][0] / UNIT_CONVERSION_FACTORS[unit2][0]
            ...
            return [types.TextContent(
                type="text",
                text=json.dumps({"error": f"Failed to validate compatibility: {str(e)}"}, indent=2)
            )]
        ...
        try:
            ...
            if "error" not in unit_info and include_examples:
                ...
                same_type_units = [u for u in UNIT_CONVERSION_FACTORS.keys() 
                                 if get_unit_type(u) == unit_type and u != unit]
                
                for example_unit in same_type_units[:3]:  # Show 3 examples
                    try:
                        factor = UNIT_CONVERSION_FACTORS[unit][0] / UNIT_CONVERSION_FACTORS[example_unit][0]
                        examples.append({
            ...
            return [types.TextContent(type="text", text=json.dumps(unit_info, indent=2))]
        except Exception as e:
            return [types.TextContent(
                type="text",
                text=json.dumps({"error": f"Failed to get unit info: {str(e)}"}, indent=2)
            )]
        ...
        try:
            ...
            if conversion_type == "energy_to_length":
                ...
                length_converted = convert_units_pdg(length_m, "m", output_units) if output_units != "m" else length_m
                ...
                time_converted = convert_units_pdg(time_s, "s", output_units) if output_units != "s" else time_s
                result["converted_value"] = time_converted
                result["formula"] = "τ = ħ/E"
                
            elif conversion_type == "mass_to_energy":
                # E = mc²
                mass_gev = convert_units_pdg(value, input_units, "GeV") if input_units != "GeV" else value
                energy_gev = mass_gev  # In natural units c = 1
                energy_converted = convert_units_pdg(energy_gev, "GeV", output_units) if output_units != "GeV" else energy_gev
                ...
                if conversion_type == "length_to_energy":
                    length_m = convert_units_pdg(value, input_units, "m") if input_units != "m" else value
                    energy_gev = (hbar_gev_s * c_m_per_s) / length_m
                    energy_converted = convert_units_pdg(energy_gev, "GeV", output_units) if output_units != "GeV" else energy_gev
                    result["converted_value"] = energy_converted
                    result["formula"] = "E = ħc/λ"
                elif conversion_type == "time_to_energy":
                    time_s = convert_units_pdg(value, input_units, "s") if input_units != "s" else value
                    energy_gev = hbar_gev_s / time_s
                    energy_converted = convert_units_pdg(energy_gev, "GeV", output_units) if output_units != "GeV" else energy_gev
                    result["converted_value"] = energy_converted
                    result["formula"] = "E = ħ/τ"
                elif conversion_type == "energy_to_mass":
                    energy_gev = convert_units_pdg(value, input_units, "GeV") if input_units != "GeV" else value
                    mass_gev = energy_gev  # In natural units c = 1
                    mass_converted = convert_units_pdg(mass_gev, "GeV", output_units) if output_units != "GeV" else mass_gev
                    result["converted_value"] = mass_converted
                    result["formula"] = "m = E/c² (c = 1 in natural units)"
                else:
                    return [types.TextContent(
                        type="text",
                        text=json.dumps({"error": f"Unknown conversion type: {conversion_type}"}, indent=2)
                    )]
            ...
            return [types.TextContent(
                type="text",
                text=json.dumps({"error": f"Natural unit conversion failed: {str(e)}"}, indent=2)
            )]
        ...
        try:
            if category == "all":
                ...
                return [types.TextContent(
                    type="text",
                    text=json.dumps({"error": f"Unknown category: {category}"}, indent=2)
                )]
            ...
            return [types.TextContent(
                type="text",
                text=json.dumps({"error": f"Failed to get common conversions: {str(e)}"}, indent=2)
            )]
