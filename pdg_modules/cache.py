        ...
        async with self._lock:
            ...
            for key, entry in list(self._cache.items())[:10]:  # Show top 10
                entries_info.append({
                    ...
                    "expires_in": max(0, (entry.created_at + entry.ttl) - time.time()),
                })
...
class CacheManager:
    ...
    async def get_cached_result(self, cache_type: str, *args, **kwargs) -> Optional[Any]:
        """Get cached result for specific data type."""
        key = self._make_cache_key(cache_type, *args, **kwargs)
        return await self.memory_cache.get(key)
    
    async def cache_result(self, cache_type: str, result: Any, ttl: float = None, *args, **kwargs) -> bool:
    ...
    async def invalidate_cache_type(self, cache_type: str):
        ...
        logger.info(f"Invalidated {len(keys_to_delete)} cache entries for type: {cache_type}")
...
def _get_cache_recommendations(stats: Dict[str, Any]) -> List[str]:
    ...
    if hit_rate < 0.5:
        recommendations.append("Consider increasing cache TTL or warming cache with common queries")
