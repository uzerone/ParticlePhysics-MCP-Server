...
class PDGConnectionPool:
    ...
    async def _cleanup_idle_connections(self):
        ...
        async with self._lock:
            ...
            for connection in connections_to_remove:
                ...
                logger.debug(f"Removed idle PDG connection (idle for {connection.idle_seconds:.1f}s)")
    ...
    async def _create_connection(self) -> PDGConnection:
        ...
        while retry_count <= max_retries:
            try:
                ...
                logger.error("PDG package not installed. Please install it using: pip install pdg")
                ...
                if retry_count > max_retries:
                    ...
                    logger.error(f"Failed to create PDG connection after {max_retries} retries: {e}")
                    raise Exception(f"Failed to connect to PDG database: {e}") from e
                
                # Exponential backoff
                delay = base_delay * (config.connection.retry_backoff ** (retry_count - 1))
                logger.warning(f"PDG connection failed (attempt {retry_count}), retrying in {delay}s: {e}")
                ...
                if connection is None:
                    if len(self._connections) < self.max_connections:
                        ...
                        if self._connections:
                            ...
                            raise Exception("No PDG connections available and pool is full")
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            pool_utilization = len(self._connections) / self.max_connections if self.max_connections > 0 else 0
