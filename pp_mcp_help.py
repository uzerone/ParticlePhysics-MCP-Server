...
def format_tool_info(tool) -> Dict[str, Any]:
    ...
    tool_info = {"name": tool.name, "description": tool.description, "parameters": {}}
...
def generate_markdown_help() -> str:
    ...
    md_lines.append(
        "This document lists all available tools in the ParticlePhysics MCP Server."
    )
        ...
        for tool in tools:
            ...
            if tool_info["parameters"]:
                ...
                for param_name, param_info in tool_info["parameters"].items():
                    ...
                    if param_info["enum"]:
                        md_lines.append(
                            f"  - Allowed values: {', '.join([f'`{v}`' for v in param_info['enum']])}"
                        )
...
def generate_llm_prompt_helper() -> str:
    ...
    lines.append(
        "The ParticlePhysics MCP Server provides access to the Particle Data Group (PDG) database."
    )
    ...
    modules_info = [
        ...
        (
            "Measurement Tools - Experimental measurements",
            measurement.get_measurement_tools(),
        ),
        (
            "Particle Tools - Particle properties and quantum numbers",
            particle.get_particle_tools(),
        ),
    ...
    lines.append(
        "- **Find particle info:** Use `search_particle` with particle name/ID"
    )
    lines.append(
        "- **Get mass/lifetime:** Use `get_particle_properties` or specific measurement tools"
    )
    lines.append(
        "- **List decay modes:** Use `get_branching_fractions` or `get_decay_products`"
    )
    ...
    for module_desc, tools in modules_info:
        ...
        for tool in tools:
            ...
            required_params = [
                p for p, info in tool_info["parameters"].items() if info["required"]
            ]
            optional_params = [
                p for p, info in tool_info["parameters"].items() if not info["required"]
            ]
